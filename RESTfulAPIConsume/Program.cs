    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var jToken in releases.Children())
            {
                ...
                Console.WriteLine("Created: {0}", DateTime.Parse(release.GetValue("created_at").ToString()));
                Console.WriteLine("Author: {0}", release.GetValue("author").ToObject<Author>().Login);
                Console.WriteLine("Assets: {0}", release.GetValue("assets"));
            }
